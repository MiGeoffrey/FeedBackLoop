...
%   Use to change the default parameters  (degrees per second, degrees)
    ...
    p = inputParser;
       ...
       % roll feedback
       addParameter(p,'gain',1);                 % gain of angular drift speed due to tail angle (unit to be defined)
       addParameter(p,'instability',0);          % default value for instability
    ...
    elseif strcmp(exp, 'rollFeedback') % calls vestibular without visual feedback
    open(vw);
        % parameters
        params.gain               = p.Results.gain;         % unit to be defined
        params.instability        = p.Results.instability;  % unit to be defined
        obj = rollFeedback(params, handles);
